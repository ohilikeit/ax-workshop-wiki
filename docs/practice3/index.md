...
!!! abstract "한 줄 요약"
    실습 1·2가 "내가 매일 하는 업무를 직접 자동화"였다면, **실습 3은 "이런 형태의 일도 비개발자가 바이브 코딩으로 만들 수 있다"는 것을 체감하는 시연 실습**입니다.

실제로 자동화해야 할 업무가 손에 있어서 만드는 게 아니라, **가상의 시나리오를 하나 잡아 90분 안에 동작하는 작은 웹앱까지 도달해보는 흐름**입니다. 그래서 결과물을 곧바로 어딘가에 투입한다는 가정은 두지 않습니다 — "이 정도 규모의 자동화 도구도 손으로 못 짜는 사람이 만들 수 있다"는 감각을 가져가는 게 목표입니다.

### 시나리오 설정

!!! note "이런 일이라면"
    외부에서 받은 채용공고 **이미지**를 자사 채용 사이트의 **정해진 폼**에 옮겨 적어야 한다고 가정해봅니다. 폼은 50여 개 항목, 한 공고에 여러 모집부문이 섞여 있고, 이미지는 세로로 매우 깁니다. 손으로 채우면 1건당 10~30분쯤 걸리는, 그런 종류의 일입니다.

이 가상 시나리오를 고른 이유는 다음 네 가지가 한꺼번에 등장해서 **"바이브 코딩 한 번에 어디까지 되는가"** 를 시험해보기 좋기 때문입니다.

- 입력이 **세로로 긴 이미지** (메인 샘플은 4000px) — 한 번에 모델에 못 넣으니 자르기·이어붙이기가 필요
- 한 이미지에 **여러 사업부문·포지션이 한꺼번에 섞여 있음** (예: 공채. 메인 샘플은 직군 3 × 직무 11 = 11개 모집부문) — 부문 수·포지션 수가 공고마다 달라서 **하드코딩이 아닌 동적 분리·반복 처리**가 필요
- 출력 폼에 정해진 **선택지 카탈로그**(학력·고용형태·전형유형·복리후생 등) — 자유 텍스트가 아니라 분류
- 결과를 사람이 **바로 검수·편집**할 수 있어야 함 — 그래서 작은 웹 화면이 필요

**이 실습에서는** 위 시나리오를 작은 웹앱으로 풀어봅니다. 이미지 공고를 화면에 올리면, 우측에 추출 결과가 Excel 표 + HTML 미리보기로 뜨고 다운로드할 수 있는 형태입니다.
