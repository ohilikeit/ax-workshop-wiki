...
**← 이전:** [2. 이미지 공고 처리 모듈](stage2.md)
</div>

> 2번에서 만든 모듈은 명령어로만 돌릴 수 있습니다. 매니저가 코드 없이 사용할 수 있도록 웹 화면을 붙입니다. 좌측 사이드바에서 이미지 공고를 추가하면 메인 영역의 왼쪽엔 원본, 오른쪽엔 변환 결과가 보이는 형태입니다.
...
!!! success "이런 결과가 보이면 정상입니다"
    - 서버가 켜져있고 안내 페이지(보통 `/docs`)가 열립니다
    - 한화 공고 이미지로 테스트했을 때 추출된 데이터와 결과 파일 다운로드 주소가 응답에 들어 있습니다

!!! tip "이상하면?"
    - "서버 주소가 안 나와" → "마지막에 서버 주소와 안내 페이지 주소를 한 줄씩 보여줘"
...
매니저가 실제로 사용할 화면을 만듭니다. 너무 자세히 적기보다는 화면 구성과 동작 흐름 위주로 부탁합니다.
...
!!! quote "AI에게 이렇게 말해보세요 — 화면 만들기"
    ```text
    @AGENTS.md 참고. 백엔드를 호출하는 화면을 만들어줘.
    ...
    어떤 화면 구조로 만들지 먼저 보여주고, 내가 확인하면 만들어줘.
    한화 공고 이미지로 끝까지 동작 확인까지 부탁해.
    ```
...
1. 백엔드 띄우기 (터미널 한 개)
2. 화면 띄우기 (터미널 한 개 더)
3. 브라우저에서 화면 주소 열기 (보통 `http://localhost:3000`)
4. 왼쪽 사이드바 "이미지 공고 추가" → `data/hanwha_2026.jpg` 업로드
5. 변환 완료 후 왼쪽(원본) ↔ 오른쪽(결과 탭) 비교
6. 오른쪽 [엑셀 형식] 탭에서 셀 수정 → "엑셀 다운로드"
7. 오른쪽 [HTML 형식] 탭에서 캐치 사이트와 비슷한 모습으로 보이는지 확인
...
> "비개발자가 코드 한 줄 안 보고 자동화를 사용할 수 있는 형태가 진짜 자동화다. 매니저가 매일 들여다볼 수 있는 화면이 곧 산출물의 완성형이다."
