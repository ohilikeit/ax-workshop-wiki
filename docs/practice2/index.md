...
!!! success "실습 목표"
    - 큰 작업을 작은 단계로 쪼개서 AI와 함께 만드는 법을 익힌다
    - 각 단계마다 눈으로 결과를 확인하고 피드백하는 습관을 기른다
    - 완성된 자동화를 Skill로 만들어 재사용한다

!!! tip "핵심 관점"
    - 한 번에 다 만들려 하지 말고, 단계별로 결과를 눈으로 보며 진행한다
    - AI한테 "관찰한 규칙"을 자연어로 넘기면 코드로 바꿔준다
    - 코드를 읽을 필요 없이, 결과 이미지만 보고 피드백하면 된다

!!! danger "주의"
    이번 실습은 한 번에 완벽한 답을 만드는 것이 아니라, **시도 → 눈으로 확인 → 피드백 → 수정**을 반복하는 훈련입니다.

!!! abstract "실습 1에서 가져오는 것"
    실습 1에서 이미 경험한 세 가지를 여기서 다시 쓴다:
    
    - **관찰담**: 내가 발견한 규칙을 AI에게 말로 설명하기
    - **검증 루프**: 결과 확인 → 피드백 → 수정 반복하기
    - **Skill화**: 작업 과정을 재사용 가능하게 포장하기
    
    다른 건 하나도 같지 않다. 데이터가 다르고, 도구가 다르고, 결과물이 다르다. 하지만 AI와 협업하는 방식은 똑같다.
...
실습 1에서 AI에게 PDF를 줬더니 표를 읽어줬다. 실습 2에서는 한 단계 더 나간다: AI에게 **사진**을 보여주고 작업을 시킨다.

여기서 알아두면 좋은 것:

**AI에게 사진은 "픽셀의 격자"다.** 사람은 사진을 보면 즉시 "아 여기 문제 번호가 있네"라고 인식하지만, AI에게 사진은 숫자(픽셀 값)의 나열일 뿐이다.

**그래서 우리가 해줄 일:** AI가 문제를 찾으려면, 우리가 "문제 번호는 글자가 크다"같은 **규칙(관찰담)**을 알려줘야 한다. 그러면 AI는 글자 크기를 숫자로 비교하는 코드를 만든다.

**코드를 이해할 필요는 없다.** 우리는 결과 사진(디버그 이미지)을 보고 "여기가 이상해"라고 말해주면 된다. 실습 1에서 한 것과 완전히 같은 방식이다.
...
|:-----:|-------------|-----------|:---------:|
| 0 | AI에게 프로젝트 배경과 작업 방식을 설명한다 | AI가 내 의도를 이해했나? | 5분 |
| 1 | PDF를 페이지별 사진으로 만들고, 진짜 문제 페이지만 골라낸다 | 문제 페이지가 정확히 걸러졌나? | 15분 |
| 2 | 문제 영역을 찾아 잘라내고, 디버그 사진과 도형 추출까지 한다 | 각 문제가 정확히 한 장씩 잘렸나? | 20분 |
| 2b | 도형이 있는 문제에서 도형만 따로 잘라 저장한다 (선택) | 도형이 정확히 잘렸나? | 15분 |
| 3 | 잘라낸 문제를 텍스트로 읽어서 엑셀에 정리한다 | 문제 텍스트와 메타데이터가 맞나? | 20분 |
| 4 | 지금까지의 과정을 Skill로 만들어 재사용 가능하게 포장한다 | 다른 교재에도 다시 쓸 수 있는가? | 15분 |
