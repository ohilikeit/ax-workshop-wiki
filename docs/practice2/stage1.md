...
!!! abstract "이 단계의 목적"
    - PDF의 각 페이지를 고해상도 PNG 사진으로 변환한다
    - 목차, 안내, 해설 페이지를 걸러내고 진짜 문제 페이지만 골라낸다
    - 판정 결과를 리포트로 남겨서 내가 확인할 수 있게 한다
...
!!! success "이런 결과가 보이면 정상입니다"
    - `output/책이름/pages/` 폴더가 생겼다
    - 폴더 안에 `page_001.png`, `page_002.png`... 사진들이 있다
    - 사진을 열어보면 **글자가 또렷하게** 보인다
...
!!! info "pdfplumber가 뭘 하는 건지"
    PDF 파일 안에는 우리 눈에 보이지 않는 정보가 숨어 있다. 각 글자가 페이지의 어디에, 어떤 크기로 놓여 있는지, 어디에 선이 그려져 있는지 같은 것들이다.
    
    `pdfplumber`는 이 숨겨진 정보를 숫자로 꺼내주는 도구다.
    
    우리가 "가운데에 세로선이 있는 페이지만 골라줘"라고 말하면, AI는 pdfplumber로 선의 위치를 숫자로 읽고, 조건에 맞는 페이지를 걸러내는 코드를 만든다.
    
    **우리는 pdfplumber를 직접 쓸 필요가 없다.** "이런 도구가 있다"는 것만 알면 된다.

!!! info "왜 이 단계가 필요해요?"
    문제집 앞쪽엔 표지, 목차, 학습법 안내, 단원 소개 같은 페이지가 많습니다. 뒤에는 해설, 부록도 있죠. 이런 페이지까지 건드리면 엉뚱한 걸 문제라고 착각해서 결과가 엉망이 됩니다.

내가 문제집을 직접 넘겨보면서 발견한 규칙을 AI에게 알려줄 겁니다:

1. **진짜 문제 페이지에는 가운데에 세로로 긴 구분선**이 그려져 있다 (두 칸으로 나뉘어 있으니까)
2. **목차나 안내 페이지**에는 `Contents`, `활용법`, `해설 구성`, `비법노트` 같은 단어가 보인다
3. 진짜 문제 페이지엔 **한 페이지에 문제가 4~10개 정도** 있다
...
!!! success "이런 결과가 보이면 정상입니다"
    - `output/책이름/report.txt` 파일이 생겼다
    - 파일을 열어보면 각 페이지가 ✅ 또는 ❌ 로 표시되어 있다
    - 내가 문제집을 직접 넘겨봤을 때 진짜 문제 페이지라고 생각한 쪽이 ✅ 로 잡혀 있다
...
- [ ] `output/책이름/pages/` 폴더에 페이지별 PNG 사진이 생겼다
- [ ] 사진을 열어보면 글자가 또렷하게 보인다
- [ ] `output/책이름/report.txt` 파일이 생겼다
- [ ] 각 페이지가 ✅ 또는 ❌ 로 정확히 판정되었다
