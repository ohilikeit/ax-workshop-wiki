...
!!! abstract "이 단계의 목적"
    - 잘라낸 문제 이미지를 AI가 직접 보고 텍스트로 변환한다
    - STEP, 문항번호, 소제목, 태그, 발문, 선지를 엑셀에 정리한다
    - 수식은 LaTeX 형식으로 변환한다
...
- [ ] `output/책이름/책이름_export.xlsx` 파일이 생겼다
- [ ] 엑셀을 열면 문제 하나당 한 행으로 정리돼 있다
- [ ] STEP, 문항번호, 페이지 정보가 맞다
- [ ] 발문 칸에 문제 텍스트가 채워져 있다 (수식은 LaTeX 형태)
- [ ] 도형이 있는 문제는 그림유무가 "O"이고 도형이미지경로에 파일 경로가 있다
