...
!!! tip ""
    압축 해제된 폴더 안에는 `.agent`, `.claude`, `data` 폴더가 들어있습니다. `data/univ`가 모집요강 PDF, `data/univ_parsed`가 비어있는 Excel 템플릿, `data/columns.md`가 각 컬럼의 의미·예시를 정리해둔 명세 파일입니다 (AI가 그대로 참조하라고 만든 파일).
