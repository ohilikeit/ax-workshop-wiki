...
!!! success "이 실습의 최종 목표"
    Stage 0~3은 **Skill을 만들기 위한 재료를 모으는 과정**이었습니다. 이 Stage에서 그 재료를 묶어 **진짜로 다시 쓸 수 있는 Skill**을 만듭니다.

    - 다른 대학 PDF에도 `/recruit-parser` 한 마디로 재실행 가능해집니다
    - 실습 2까지 가지 못하더라도, 이 Stage만 끝내면 "자동화를 Skill로 포장하는 경험"은 완결됩니다
...
!!! tip "시간이 남으면"
    다른 대학 PDF에 Skill을 적용해봅니다. 시간이 없다면 실습 2에서 본격적으로 할 예정이니 넘어가도 됩니다.
