...
flowchart LR
    ...
    subgraph WORK["🤖 이번 실습: AI와 함께 자동화"]
        ...
        S2["Stage 2<br/>자동화 설계<br/><i>파싱 스크립트를 만든다</i>"]
        S3["Stage 3<br/>정답 비교<br/><i>사람이 눈으로 확인한다</i>"]
        S0 --> S1 --> S2 --> S3
    end
...
| Stage | 무엇을 하나 | 핵심 질문 |
|:-----:|------------|----------|
| 0 | AI에게 바로 시켜보며 어디서 헷갈리는지 봅니다 | 지금 이 방식으로도 어느 정도 되나? |
| 1 | 도메인 지식, 위치, 용어, 예외 규칙을 붙여줍니다 | 어떤 규칙을 설명하면 더 좋아지나? |
| 2 | 파싱 흐름을 설계하고 AI가 스스로 교차 검증하며 개발합니다 | 왜 안 되는가, 어느 경로로 가야 하는가? |
| 3 | 사람이 직접 확인하고 정답 Excel과 비교합니다 | 실제로 맞는가, 반복 오류는 무엇인가? |
| 4 | **지금까지의 흐름을 재사용 가능한 Skill로 만듭니다** | 다른 대학 PDF에도 한 마디로 돌릴 수 있나? |

!!! tip "왜 Stage 4(Skill화)가 중요한가요?"
    Stage 0~3은 **이번 한 번** PDF를 Excel로 만드는 과정입니다. Stage 4에서 이 흐름을 Skill로 포장하면, 다음에는 `/recruit-parser 연세대 PDF` 같은 한 마디로 똑같은 작업을 끝낼 수 있습니다. 자동화의 진짜 가치는 **반복 실행**에서 나오므로, 시간이 빠듯하면 Stage 3보다 Stage 4를 우선하세요.
